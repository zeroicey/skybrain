...
import { Users, TrendingUp, AlertTriangle, Clock, Home, Camera, Wifi, Activity } from 'lucide-react'
import { useDormitoryStore } from '@/stores/dormitory-store'
import { generateGenderDistribution, dormitoryCameras } from '@/data/mock-dormitories'
...
export default function DormitoryPage() {
  ...
  const [selectedCamera, setSelectedCamera] = useState(dormitoryCameras[0].id)
  ...
  const camerasFiltered = useMemo(() => dormitoryCameras.filter((c) => c.dormitoryId === selectedDormitoryId), [selectedDormitoryId])
  const pieData = useMemo(() => generateGenderDistribution(), [])
      ...
      <div className="flex gap-3">
        <div className="w-[280px] flex-shrink-0 flex flex-col justify-around overflow-hidden">
          ...
          <div className="min-h-[70px]">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">今日进出</h3>
            <div className="h-[calc(100%-20px)] flex items-center justify-center">
          ...
          <div className="min-h-[100px]">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">楼层人数</h3>
            <div className="h-[calc(100%-20px)]">
              <div className="space-y-2">
                {areaHeatmap.slice(0, 6).map((area) => (
                  <div key={area.id} className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground w-8 truncate">{area.name}</span>
          ...
          <div className="min-h-[180px] flex flex-col">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">人员分布</h3>
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="relative rounded-lg overflow-hidden bg-slate-900 aspect-video">
            <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-slate-600 text-xs mt-1">{camerasFiltered.find((c) => c.id === selectedCamera)?.name}</p>
              </div>
          ...
          <div className="flex gap-3 min-h-[180px]">
            <div className="flex-1">
              <h3 className="text-xs font-medium mb-1 text-muted-foreground">24小时归寝趋势</h3>
              <div className="h-[calc(100%-20px)]">
                <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="w-[240px] flex-shrink-0 flex flex-col">
          <div className="flex-1 flex items-center">
            ...
            <div>
              <div className="text-xs text-muted-foreground">当前在寝</div>
          ...
          <div className="flex-1">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">归寝高峰</h3>
            <div className="h-[calc(100%-20px)] flex items-center justify-center">
              <div className="text-center">
          ...
          <div className="flex-1">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">归寝率</h3>
            <div className="h-[calc(100%-20px)] flex items-center justify-center">
              <div className="text-center">
                <Home className="w-6 h-6 text-green-500 mx-auto mb-1" />
                <span className="text-xl font-bold">{statCardData.returnRate}%</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">在寝状态</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-green-500" />
                <span className="text-sm">充足 (&gt;60%)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-yellow-500" />
                <span className="text-sm">一般 (30-60%)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-red-500" />
                <span className="text-sm">较少 (&lt;30%)</span>
              </div>
            </div>
          </div>
        </div>
