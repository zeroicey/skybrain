...
import { Users, TrendingUp, AlertTriangle, Clock, Star, Camera, Wifi, Activity } from 'lucide-react'
import { useShopStore } from '@/stores/shop-store'
import { generateSourceDistribution, shopCameras } from '@/data/mock-shops'
...
export default function ShopsPage() {
  ...
  const shopCamerasFiltered = useMemo(
    () => shopCameras.filter((c) => c.shopId === selectedShopId),
    [selectedShopId]
  )

  const pieData = useMemo(() => generateSourceDistribution(), [])
    ...
    <div className="h-full overflow-auto p-3 no-scrollbar">
      <div className="flex gap-3">
              ...
              <div className="relative w-20 h-20">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle
                    ...
                    r="34"
                    fill="none"
                    stroke={congestionIndex < 40 ? COLORS.success : congestionIndex < 70 ? COLORS.warning : COLORS.danger}
                    strokeWidth="8"
                    strokeLinecap="round"
                  ...
                  <div key={area.id} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 h-1.5 bg-secondary/20 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${area.density}%`,
                          backgroundColor: area.density > 70 ? COLORS.danger : area.density > 40 ? COLORS.warning : COLORS.success,
                        }}
                      />
                    </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="flex gap-3 min-h-[180px]">
            <div className="flex-1">
              <h3 className="text-xs font-medium mb-1 text-muted-foreground">24小时客流量</h3>
              <div className="h-[calc(100%-20px)]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={hourlyTrend.filter((_, i) => i % 4 === 0)} margin={{ left: -15, right: 0, top: 5, bottom: 0 }}>
                    ...
                    <XAxis dataKey="time" tick={{ fontSize: 10 }} tickFormatter={(value) => value.split(':')[0] + '时'} stroke="#64748b" />
        ...
        <div className="w-[240px] flex-shrink-0 flex flex-col">
          ...
          <div className="flex-1">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">最热门商铺</h3>
            <div className="h-[calc(100%-20px)] flex items-center justify-center">
              <div className="text-center">
                <Star className="w-6 h-6 text-yellow-500 mx-auto mb-1" />
                <span className="text-xl font-bold">{statCardData.hotShop}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1">
            <h3 className="text-xs font-medium mb-1 text-muted-foreground">客流状态</h3>
